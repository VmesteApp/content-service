...
async def create_tag(request: Request, new_tag: CreateTag, session: Session = Depends(get_db)):
...
async def update_tag(request: Request, up_tag: UpdateTag, session: Session = Depends(get_db)):
...
async def delete_tag(request: Request, del_tag: DeleteTag, session: Session = Depends(get_db)):
    if request.state.role == "admin":
